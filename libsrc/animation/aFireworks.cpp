	...
	*/
{
	...
	float speed = rand() % 60 + 60;
	float launchAngle = (rand() % 30 + 80) * RAD;

	float dirX = cos(launchAngle);
	float dirY = abs(sqrt(1.f - dirX*dirX));
	...
	stateVals[0] = posx;
	//stateVals[1] posy is always 0
	//stateVals[2] posz doesn't matter since we're doing 2D
	
	stateVals[3] = dirX * speed;
	stateVals[4] = dirY * speed;
	//stateVals[5] z doesn't matter
	...
	stateVals[10] = ROCKET_LIFESPAN; // time to live
	...
	// The default mass of the rocket is 50.0, it should explode for a total of explosionCount = TOTALEXPLOSIONS time steps
{
	...
	numSparks = rand() % 50 + 10;
	
	float speed = rand() % 20 + 20;
	...
	for (int i = 0; i < numSparks; i++)
	{
		...
		float dirX = cos((float)i / (float)numSparks * PI * 2.f);
		float dirY = sin((float)i / (float)numSparks * PI * 2.f);
		stateVals[3] = dirX * speed + rocketVelx;
		stateVals[4] = dirY * speed + rocketVely;
		// stateVals[5] z doesn't matter
		...
		stateVals[10] = SPARK_LIFESPAN; // time to live
...
void AFireworks::update(float deltaT, int extForceMode)
{
	...
	for (int i = 0; i < rockets.size(); i++)
	{
		...
		else
		{
			...
			{
				std::vector<float> state = pRocket->getState();
				explode(state[0], state[1], state[2], state[3], state[4], state[5], pRocket->m_color);
			}
		}
